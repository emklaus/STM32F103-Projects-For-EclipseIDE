...
// ***********************************************************************
// lcd_write_4bits a byte to the LCD in 4 bit mode
// ***********************************************************************
void lcd_write_4bits(unsigned char b)
{
  ...
  if(b & 0x01)
	GPIO_SetBits(LCD_CTL_PORT, LCD_D4);   // Set D4 HIGH
  else
	GPIO_ResetBits(LCD_CTL_PORT, LCD_D4); // Set D4 LOW

  if(b & 0x02)
	GPIO_SetBits(LCD_CTL_PORT, LCD_D5);   // Set D5 HIGH
  else
	GPIO_ResetBits(LCD_CTL_PORT, LCD_D5); // Set D5 LOW

  if(b & 0x04)
	GPIO_SetBits(LCD_CTL_PORT, LCD_D6);   // Set D6 HIGH
  else
	GPIO_ResetBits(LCD_CTL_PORT, LCD_D6); // Set D6 LOW

  if(b & 0x08)
	GPIO_SetBits(LCD_CTL_PORT, LCD_D7);   // Set D7 HIGH
  else
	GPIO_ResetBits(LCD_CTL_PORT, LCD_D7); // Set D7 LOW
